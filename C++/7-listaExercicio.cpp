...
void pesqMenorQue(Lista* ptrLista, float preco);
void pesqDescricao(Lista* ptrLista, string descricao);
...
int main() {
	...
	// Inserir Ordenado
	//-------------------------------------------------
	inserirListaOrdenada(pLista, 30, "Omo", 70.5, "Fab Detergente");
	inserirListaOrdenada(pLista, 10, "Pão", 51.0, "Fab Padeiro");
	...
	// pesqMenorQue(pLista, 50);


    pesqDescricao(pLista, "Leite");
...
void pesqDescricao(Lista* ptrLista, string descricao) {
	...
	while (ptrNoAtual != NULL) {
        if (ptrNoAtual->dados.descricao == descricao) {
            achou = true;
