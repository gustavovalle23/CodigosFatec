...
void concatenarListas(Lista *ptrDiscBD, Lista *ptrDiscED, Lista *ptrCurso);
...
int main() {
	...
	inserirListaOrdenada(pDiscED, 20, "Maria", 8.5);
    ...
    concatenarListas(pDiscBD, pDiscED, pCurso);
    exibirLista(pCurso);
...
void concatenarListas(Lista *ptrDiscBD, Lista *ptrDiscED, Lista *ptrCurso) {
    ...
    ptrNoAtual = ptrDiscBD->inicio;

	// Localiza a posição de inserção
	while (ptrNoAtual != NULL) {

        inserirListaFim(ptrCurso,ptrNoAtual->dados.matricula, 
                        ptrNoAtual->dados.nome, ptrNoAtual->dados.media);
